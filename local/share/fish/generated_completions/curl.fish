...
complete -c curl -s '#' -l progress-bar --description 'Make curl display progress as a simple progress bar instead of the standard, …'
...
complete -c curl -l create-dirs --description 'When used in conjunction with the -o option, curl will create the necessary l…'
complete -c curl -l crlf --description 'Convert LF to CRLF in upload.  Useful for MVS (OS/390).   (SMTP added in 7.'
complete -c curl -l crlfile --description '(HTTPS/FTPS) Provide a file using PEM format with a Certificate Revocation Li…'
...
complete -c curl -l data-raw --description '(HTTP) This posts data similarly to --data but without the special interpreta…'
complete -c curl -l data-urlencode --description '(HTTP) This posts data, similar to the other --data options with the exceptio…'
complete -c curl -l delegation --description 'Set LEVEL to tell the server what it is allowed to delegate when it comes to …'
...
complete -c curl -l disable-epsv --description '(FTP) Tell curl to disable the use of the EPSV command when doing passive FTP…'
complete -c curl -l dns-interface --description 'Tell curl to send outgoing DNS requests through <interface>.'
complete -c curl -l dns-ipv4-addr --description 'Tell curl to bind to <ip-address> when making IPv4 DNS requests, so that the …'
complete -c curl -l dns-ipv6-addr --description 'Tell curl to bind to <ip-address> when making IPv6 DNS requests, so that the …'
...
complete -c curl -s E -l cert --description '(SSL) Tells curl to use the specified client certificate file when getting a …'
complete -c curl -l engine --description 'Select the OpenSSL crypto engine to use for cipher operations.'
complete -c curl -l environment --description '(RISC OS ONLY) Sets a range of environment variables, using the names the -w …'
complete -c curl -l egd-file --description '(SSL) Specify the path name to the Entropy Gathering Daemon socket.'
...
complete -c curl -l cert-type --description '(SSL) Tells curl what certificate type the provided certificate is in.'
complete -c curl -l cacert --description '(SSL) Tells curl to use the specified certificate file to verify the peer.'
complete -c curl -l capath --description '(SSL) Tells curl to use the specified certificate directory to verify the pee…'
complete -c curl -l pinnedpubkey --description '(SSL) Tells curl to use the specified public key file (or hashes) to verify t…'
complete -c curl -l cert-status --description '(SSL) Tells curl to verify the status of the server certificate by using the …'
...
complete -c curl -s F -l form --description '(HTTP) This lets curl emulate a filled-in form in which a user has pressed th…'
...
complete -c curl -l form-string --description '(HTTP) Similar to --form except that the value string for the named parameter…'
...
complete -c curl -s I -l head --description '(HTTP/FTP/FILE) Fetch the HTTP-header only! HTTP-servers feature the command …'
...
complete -c curl -s k -l insecure --description '(SSL) This option explicitly allows curl to perform "insecure" SSL connection…'
...
complete -c curl -s l -l list-only --description '(FTP) When listing an FTP directory, this switch forces a name-only view.'
complete -c curl -s L -l location --description '(HTTP/HTTPS) If the server reports that the requested page has moved to a dif…'
...
complete -c curl -l local-port --description 'Set a preferred number or range of local port numbers to use for the connecti…'
complete -c curl -l location-trusted --description '(HTTP/HTTPS) Like -L, --location, but will allow sending the name + password …'
...
complete -c curl -l netrc-file --description 'This option is similar to --netrc, except that you provide the path (absolute…'
...
complete -c curl -l no-keepalive --description 'Disables the use of keepalive messages on the TCP connection, as by default c…'
...
complete -c curl -s p -l proxytunnel --description 'When an HTTP proxy is used (-x, --proxy), this option will cause non-HTTP pro…'
complete -c curl -s P -l ftp-port --description '(FTP) Reverses the default initiator/listener roles when connecting with FTP.'
complete -c curl -l pass --description '(SSL/SSH) Passphrase for the private key  If this option is used several time…'
...
complete -c curl -l proto --description 'Tells curl to use the listed protocols for its initial retrieval.'
...
complete -c curl -l proto-default --description 'Tells curl to use protocol for any URL missing a scheme name.   Example:  .'
...
complete -c curl -s q --description 'If used as the first parameter on the command line, the curlrc config file wi…'
...
complete -c curl -l random-file --description '(SSL) Specify the path name to file containing what will be considered as ran…'
...
complete -c curl -s S -l show-error --description 'When used with -s it makes curl show an error message if it fails.'
...
complete -c curl -l ssl-allow-beast --description '(SSL) This option tells curl to not work around a security flaw in the SSL3 a…'
...
complete -c curl -o post303 --description '.'
