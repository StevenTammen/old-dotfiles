...
complete -c git-log -l perl-regexp --description 'Consider the limiting patterns to be Perl-compatible regular expressions.'
...
complete -c git-log -l all --description 'Pretend as if all the refs in refs/ are listed on the command line as <commit…'
...
complete -c git-log -l left-only -l right-only --description 'List only commits on the respective side of a symmetric range, i. e.'
...
complete -c git-log -l reverse --description 'Output the commits in reverse order.  Cannot be combined with --walk-reflogs.'
...
complete -c git-log -l color --description 'auto settings of the former if we are going to a terminal).  auto alone (i. e.'
