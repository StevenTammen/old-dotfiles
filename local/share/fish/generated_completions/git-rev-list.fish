...
complete -c git-rev-list -l perl-regexp --description 'Consider the limiting patterns to be Perl-compatible regular expressions.'
...
complete -c git-rev-list -l all --description 'Pretend as if all the refs in refs/ are listed on the command line as <commit…'
...
complete -c git-rev-list -l left-only -l right-only --description 'List only commits on the respective side of a symmetric range, i. e.'
...
complete -c git-rev-list -l reverse --description 'Output the commits in reverse order.  Cannot be combined with --walk-reflogs.'
...
complete -c git-rev-list -l color --description 'auto settings of the former if we are going to a terminal).  auto alone (i. e.'
